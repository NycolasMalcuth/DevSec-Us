...
class GameMap {
  ...
  load() {
    return new Promise((resolve, reject) => {
      this.img.onload = () => { this.ready = true; resolve(); };
...
class Player {
  ...
  update(dt, keys) {
    let moved = false;
    if (keys['w'] || keys['arrowup']) { this.y -= this.speed * dt; moved = true; }
    if (keys['s'] || keys['arrowdown']) { this.y += this.speed * dt; moved = true; }
    if (keys['a'] || keys['arrowleft']) { this.x -= this.speed * dt; moved = true; }
    if (keys['d'] || keys['arrowright']) { this.x += this.speed * dt; moved = true; }
    ...
    if (moved && this._lastEmit > 80 && this.socket && this.id) {
      this.socket.emit('pos_update', { room: this.room, player_id: this.id, x: this.x, y: this.y });
      this._lastEmit = 0;
    }
  }
  ...
  handleServerEvent(name, data) {
    if (name === 'player_joined') {
      ...
      for (const pid in data.players) {
        ...
        this.otherPlayers[pid] = { x: data.players[pid].x, y: data.players[pid].y };
      }
...
function startGame(roomId) {
  ...
  // Eventos socket
  socket.on('connect', () => {
    info.textContent = 'Conectado ao servidor. Entrando na sala ' + roomId + '...';
  ...
  socket.on('joined', data => {
    ...
    info.textContent = 'Use WASD ou setas para andar. Jogadores na sala: ' + Object.keys(player.otherPlayers).length;
  });

  socket.on('player_joined', data => {
    player.handleServerEvent('player_joined', data);
    info.textContent = 'Novo jogador entrou — total: ' + (1 + Object.keys(player.otherPlayers).length);
  });
  ...
  socket.on('player_left', data => {
    ...
    info.textContent = 'Um jogador saiu — total: ' + (1 + Object.keys(player.otherPlayers).length);
  });
